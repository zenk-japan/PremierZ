		...
		<div id="id_div_list_menu_table">
			<table id="id_table_list_menu">
				<tr id="id_tr_list_menu_top">
					<td class="c_td_list_menu" id="id_th_list_menu_invalid">&nbsp;</td>
					<td class="c_td_list_menu" id="id_th_list_menu_name">名前</td>
					<td class="c_td_list_menu" id="id_th_list_menu_comp">会社</td>
					<td class="c_td_list_menu" id="id_th_list_menu_group">グループ</td>
				</tr>
			{foreach from=$ar_list_menu item=list_menu_item name=fe_list_menu}
				<tr id="id_tr_list_menu{$smarty.foreach.fe_list_menu.iteration}" class="c_tr_list_menu">
					...
					{* 有効無効表示 *}
						...
						{/if}
						<input id="id_usrid_list_menu{$smarty.foreach.fe_list_menu.iteration}" type="hidden" value="{$list_menu_item.USER_ID}" />
					</td>
					<td nowrap class="c_td_list_menu_user" title="{$list_menu_item.NAME}">
					{* 氏名 *}
						{$list_menu_item.NAME_SHORT}
					</td>
					<td nowrap class="c_td_list_menu_company" title="{$list_menu_item.COMPANY_NAME}">
					{* 会社 *}
						{if isset($list_menu_item.COMPANY_NAME)}{$list_menu_item.COMPANY_NAME_SHORT}{else}&nbsp;{/if}
					</td>
					<td nowrap class="c_td_list_menu_group" title="{$list_menu_item.GROUP_NAME}">
					{* グループ *}
						{if isset($list_menu_item.GROUP_NAME)}{$list_menu_item.GROUP_NAME_SHORT}{else}&nbsp;{/if}
					</td>
				</tr>
			{foreachelse}
				<tr>
					<td colspan=4>条件に合うユーザーが存在しません</td>
				</tr>
			{/foreach}
			</table>
		</div>
		<div id="id_div_list_menu_ckb">
			<table id="id_table_list_menu_ckb">
				<tr>
