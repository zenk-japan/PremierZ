		...
		<div id="id_div_list_menu_table">
			<table id="id_table_list_menu">
				<tr id="id_tr_list_menu_top">
					...
					<td class="c_td_list_menu" id="id_td_list_menu_workplace">作業拠点名</td>
					<td class="c_td_list_menu" id="id_td_list_menu_comp">会社</td>
				</tr>
			{foreach from=$ar_list_menu item=list_menu_item name=fe_list_menu}
				<tr id="id_tr_list_menu{$smarty.foreach.fe_list_menu.iteration}" class="c_tr_list_menu">
					...
					{* 有効無効表示 *}
						...
						{/if}
						<input id="id_baseid_list_menu{$smarty.foreach.fe_list_menu.iteration}" type="hidden" value="{$list_menu_item.BASE_ID}" />
					</td>
					<td class="c_td_list_menu_workplace" title="{$list_menu_item.BASE_NAME}">
						{$list_menu_item.BASE_NAME_SHORT}
					</td>
			...
			{foreachelse}
				<tr>
					<td colspan=4>条件に合う作業場所が存在しません</td>
				</tr>
			{/foreach}
			</table>
		</div>
