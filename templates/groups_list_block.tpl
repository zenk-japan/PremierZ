		...
		<div id="id_div_list_menu_table">
			<table id="id_table_list_menu">
				<tr id="id_tr_list_menu_top">
					...
					<td class="c_td_list_menu" id="id_td_list_menu_group">グループ</td>
			...
			{foreach from=$ar_list_menu item=list_menu_item name=fe_list_menu}
				<tr id="id_tr_list_menu{$smarty.foreach.fe_list_menu.iteration}" class="c_tr_list_menu">
					...
					{* 有効無効表示 *}
						...
						{/if}
						<input id="id_groupid_list_menu{$smarty.foreach.fe_list_menu.iteration}" type="hidden" value="{$list_menu_item.GROUP_ID}" />
					</td>
					<td class="c_td_list_menu_group" title="{$list_menu_item.GROUP_NAME}">
						{$list_menu_item.GROUP_NAME}
					</td>
			...
			{foreachelse}
				<tr>
					<td colspan=4>条件に合うグループが存在しません</td>
				</tr>
			{/foreach}
			</table>
		</div>
